...
import divergenceData from '../models/divergence.js';
import { checkForMTFAlert } from '../models/mtfAlert.js';
...
export const calculate = async (coin, interval, dataType) => {
			...
			for (let j = checkCandles.length - 1; j >= 0; j--) {
				if (rsiEntry[i].endTimeStamp == checkCandles[j]) {
					if (rsiEntry[i].rsi14Val < rsi14) {
						...
						console.log('BULLISH DIVERGENCE 14');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
								startTimeStamp: data[0].startTimeStamp,
			...
			for (let j = rsiCheckValues.length - 1; j >= 0; j--) {
				if (
					...
					if (data[i].lowVal < data[0].lowVal) {
						...
						console.log('HIDDEN BULLISH DIVERGENCE RSI14');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
								...
								endTimeStamp: data[0].endTimeStamp,
								divExists: true,
								divType: 'HIDDEN BULLISH DIVERGENCE',
								divIndicator: 'RSI14',
								dataType,
							})
							.then(() => {
	...
	if (rsiEntry.length >= 45 && data[0].candleType == 'GREEN') {
		console.log('Candles in past 20');
		console.log(coin + ' ' + dataType + ' ' + interval + '\n');
		...
		for (let i = 44; i >= 0; i--) {
			for (let j = checkCandles.length - 1; j >= 0; j--) {
				if (rsiEntry[i].endTimeStamp == checkCandles[j]) {
					if (rsiEntry[i].rsi14Val > rsi14) {
						console.log(rsiEntry[i].endTimeStamp + ' ' + rsiEntry[i].rsi14Val);
						console.log('BEARISH DIVERGENCE RSI14');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
			...
			for (let j = rsiCheckValues.length - 1; j >= 0; j--) {
				if (
					...
					if (data[i].highVal > data[0].highVal) {
						...
						console.log('HIDDEN BEARISH DIVERGENCE RSI14');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
								...
								endTimeStamp: data[0].endTimeStamp,
								divExists: true,
								divType: 'HIDDEN BEARISH DIVERGENCE',
								divIndicator: 'RSI14',
								dataType,
							})
							.then(() => {
	...
	if (data.length >= 15) {
		...
		if (
			...
			rsi14Alert =
				...
				rsi14;
			sendMessageToTelegramBot(rsi14Alert);
		} else if (rsiEntry.length >= 14 && rsi14 < smarsi14) {
			rsi14Alert =
				'For ' +
				coin +
				' in ' +
				...
				if (rsiEntry[i].endTimeStamp == checkCandles[j]) {
					if (rsiEntry[i].rsi2Val < rsi2) {
						...
						checkForMTFAlert(
							...
							dataType,
							'BUDRSI2',
							rsiEntry[i].endTimeStamp,
							data[0].endTimeStamp
						);
						console.log('BULLISH DIVERGENCE RSI2');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
			...
			for (let j = rsiCheckValues.length - 1; j >= 0; j--) {
				if (
					...
					if (data[i].lowVal < data[0].lowVal) {
						...
						console.log('HIDDEN BULLISH DIVERGENCE RSI2');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
			...
			for (let j = checkCandles.length - 1; j >= 0; j--) {
				if (rsiEntry[i].endTimeStamp == checkCandles[j]) {
					if (rsiEntry[i].rsi2Val > rsi2) {
						...
						console.log('BEARISH DIVERGENCE RSI2');
						await divergenceData
							.create({
								symbol: data[0].symbol,
								interval: data[0].interval,
		...
		for (let i = 20; i > 0; i--) {
			for (let j = rsiCheckValues.length - 1; j >= 0; j--) {
				if (
					...
					if (data[i].highVal > data[0].highVal) {
						console.log(data[i].endTimeStamp + ' ' + data[i].highVal);
						console.log('HIDDEN BEARISH DIVERGENCE RSI2');
						checkForMTFAlert(
							coin,
							interval,
							dataType,
	...
	if (rsi2 < 10 && data.length >= 15) {
		rsi2Alert =
			...
			rsi2;
		checkForMTFAlert(coin, interval, dataType, 'RSI<10', rsi2);
	}
	if (rsi2 > 90 && data.length >= 15) {
		rsi2Alert =
			...
			rsi2;
		checkForMTFAlert(coin, interval, dataType, 'RSI>90', rsi2);
	}
	...
	if (cprEntry.length !== 0) {
		...
		if (cprEntry[0].cprTcVal < cpr_bc && interval == '1d') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (cprEntry[0].cprBcVal > cpr_tc && interval == '1d') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (cprEntry[0].cprTcVal < cpr_bc && interval == '1w') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (cprEntry[0].cprBcVal > cpr_tc && interval == '1w') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (cprEntry[0].cprTcVal < cpr_bc && interval == '1M') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (cprEntry[0].cprBcVal > cpr_tc && interval == '1M') {
			cprCamAlert =
				'For ' +
				coin +
				' in ' +
	...
	if (data.length >= 9) {
		...
		if (
			newEma9PriceVal > newEma13PriceVal &&
			newEma9PriceVal != 0 &&
			newEma13PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma13PriceVal > newEma21PriceVal &&
			newEma13PriceVal != 0 &&
			newEma21PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma21PriceVal > newEma34PriceVal &&
			newEma21PriceVal != 0 &&
			newEma34PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma34PriceVal > newEma55PriceVal &&
			newEma34PriceVal != 0 &&
			newEma55PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma55PriceVal > newEma89PriceVal &&
			newEma55PriceVal != 0 &&
			newEma89PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma55PriceVal > newEma100PriceVal &&
			newEma55PriceVal != 0 &&
			newEma100PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma89PriceVal > newEma200PriceVal &&
			newEma89PriceVal != 0 &&
			newEma200PriceVal != 0
		) {
			emaAlert =
		...
		if (
			newEma100PriceVal > newEma200PriceVal &&
			newEma100PriceVal != 0 &&
			newEma200PriceVal != 0
		) {
			emaAlert =
		...
		if (data[0].closeVal > newEma9PriceVal && newEma9PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma9PriceVal &&
		...
		if (data[0].closeVal > newEma13PriceVal && newEma13PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma13PriceVal &&
			...
			emaAlert =
				...
				newEma13PriceVal;
			sendMessageToTelegramBot(emaAlert);
		} else if (data[0].closeVal > newEma21PriceVal && newEma21PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (data[0].closeVal < newEma21PriceVal && newEma21PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma21PriceVal &&
		...
		if (data[0].closeVal > newEma34PriceVal && newEma34PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma34PriceVal &&
		...
		if (data[0].closeVal > newEma55PriceVal && newEma55PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma55PriceVal &&
		...
		if (data[0].closeVal > newEma89PriceVal && newEma89PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma89PriceVal &&
		...
		if (data[0].closeVal > newEma100PriceVal && newEma100PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma100PriceVal &&
		...
		if (data[0].closeVal > newEma200PriceVal && newEma200PriceVal != 0) {
			emaAlert =
				'For ' +
				coin +
				' in ' +
		...
		if (
			data[0].closeVal > newEma200PriceVal &&
